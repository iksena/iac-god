...
class ResearchRecorder:
    ...
    def record_rag_tool_call(
        ...
        2. rag_tool_data_flow.txt  (overwrite)
           Human-readable snapshot of the most recent tool invocation.
           Shows the complete data flow: input queries, template size, and the
           full schema context returned by the tool. Overwritten on every call
           so it always reflects the latest invocation without growing unboundedly.
        ...
        # Sink 2: human-readable overwrite — latest invocation data flow only
    ...
    def _write_rag_tool_data_flow(
        ...
        """Overwrite rag_tool_data_flow.txt with the latest tool invocation.

        Shows the complete data flow through retrieve_schema_context:
          INPUT  : retrieval queries generated by the LLM
          INPUT  : template YAML passed as Neo4j seed (full text)
          OUTPUT : schema context string returned by the hybrid RAG pipeline

        This file is always overwritten (open mode 'w') so it reflects only
        the most recent invocation. It is never touched by save_iteration_snapshot
        or _write_agent_history. For the full cross-iteration audit trail, see
        ...
        content = (
            ...
            f"{reasoning_section}\n"
            f"\n"
            f"[INPUT: retrieval queries ({len(retrieval_queries)})]\n"
            f"{'-' * 72}\n"
            f"{queries_str}\n"
            f"\n"
            f"[INPUT: template_yaml ({len(template_yaml)} chars)]\n"
            f"{'-' * 72}\n"
            f"{template_yaml}\n"
            f"\n"
        ...
        data_flow_path.write_text(content, encoding="utf-8")
