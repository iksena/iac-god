...
def execute_terraform_retrieval(
    ...
    permanently disabled — mirrors cfn_hybrid_rag.execute_hybrid_retrieval().
    identified_resources comes solely from seed_resources (deterministic
    parsing of the template), never from semantic search. retrieval_queries
    (HyDE) is accepted for call-site compatibility with retriever_agent but
    is unused: Stage 2 keys on resource type, not on free-text queries.
    ...
    print("[TF RAG] ABLATION MODE: Skipping ChromaDB semantic search (no-dense-rag).")
