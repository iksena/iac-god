...
def execute_terraform_retrieval(
    ...
    is permanently disabled — mirrors cfn_hybrid_rag.execute_hybrid_retrieval().
    The query-generation LLM call (HyDE) upstream in retriever_agent is
    untouched, so this isolates the Graph RAG contribution from the rest of
    the retrieval pipeline. Called by retriever_agent() when iac_type == "terraform".
    ...
    print("[TF RAG] ABLATION MODE: Skipping Neo4j graph lookup (no-graph-rag).")
    schema_blocks: list[str] = []
